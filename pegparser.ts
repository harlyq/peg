...
(function(window) {
    ...
    PEGParser.prototype.setOptions = function(options: PEGOptions) {
        if (!options)
            return;
    ...
    PEGParser.prototype.parse = function(str: string): string {
    ...
    PEGParser.prototype.applyParams = function(definition: PEGDefinition, params, results) {
    ...
    PEGParser.prototype.parseInternal = function(str: string, i: number, definition: PEGDefinition): PEGInfo {
        ...
        switch (definition.type) {
            case 'definition':
                var childDefinition: PEGDefinition = this.define[definition.defn];
                if (!childDefinition) {
                    debugger; // unknown definition
                    break;
                }
            ...
            case 'zero or more':
                var k: number = i,
                    result: any[],
                    params = [],
                    childDefinition: PEGDefinition = definition.child;
